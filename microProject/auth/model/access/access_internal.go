...
func (s *service) createTokenClaims(subject *Subject) (m *jwt.StandardClaims,err error)  {
	...
	m = &jwt.StandardClaims{
		ExpiresAt:now.Add(tokenExpiredDate).Unix(),//token过期时间
		NotBefore:now.Unix(),
		Id:subject.ID,
...
//保存token到redis中
func (s *service) saveTokenToCachae(subject *Subject,val string)(err error)  {
	fmt.Println("redisKey",tokenIDKeyPrefix+subject.ID)
	if err = redisCli.Set(tokenIDKeyPrefix+subject.ID,val,tokenExpiredDate).Err();err != nil{
...
func (s *service) delTokenFromCache(subject *Subject) (err error)  {
	if err = redisCli.Del(tokenIDKeyPrefix+subject.ID).Err();err != nil {
...
func (s *service) getTokenFromCache(subject *Subject)(token string,err error)  {
	fmt.Println("redisKey",tokenIDKeyPrefix+subject.ID)
	tokenCached,err := redisCli.Get(tokenIDKeyPrefix+subject.ID).Result()
	if err != nil {
